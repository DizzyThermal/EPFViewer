...
var HEADER_SIZE: int = 0x4
var FRAME_SIZE: int = 0x2

var effect_count: int = 0
var effects: Dictionary[int, NTK_Effect] = {}
...
func decode_bytes(
	...
	var decode_array: Array[int] = [ 75, 25, 31, 29, 26, 9, 12, 12, 83, 73, 19, 17, 29, 23, 6, 29, 9, 6, 8, 27, 28, 1, 30, 29, 3, 5, 9 ]
	for i in range(26):
		decode_array[i + 1] ^= decode_array[i]
	
	var pre: int = -0x1234568 - offset
	pre = pre & 0x00000000FFFFFFFF
	var reverse_idx: int = pre % 0x1B
	
	for i in range(8):
		var unsigned_byte: int = encoded_bytes.decode_u8(i)
	...
	var first: int = encoded_bytes.decode_u32(0) & 0xFFFFFFFF
	var second: int = encoded_bytes.decode_u32(4) & 0xFFFFFFFF
...
func _init(file, decode=true):
	...
	if decode:
		...
		for i in range(file_size / 8):
			var decoded_bytes: PackedByteArray = decode_bytes(offset, read_bytes(file_position, 8))
			file_position += 8
			for j in range(4):
	...
	for i in range(effect_count):
		var effect_index: int = read_u32(file_position)
		file_position += 4
		...
		for j in range(frame_count):
			var frame_index: int = read_s32(file_position)
			file_position += 4
			if frame_index == -1:
				while frame_index != i + 1:
					if frame_index == -1:
						...
						var frame_delay: int = read_s32(file_position)
						file_position += 4
						var palette_index: int = read_u32(file_position)
						file_position += 4
						var unknown: int = read_u32(file_position)
						file_position += 4
