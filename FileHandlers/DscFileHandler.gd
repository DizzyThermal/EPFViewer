...
func _init(file):
	super(file)
	var file_position: int = HEADER_SIZE

	part_count = read_u32(file_position)
	file_position += 4
	for i in range(part_count):
		...
		# Unknown Bytes
		var part_unknown_bytes := PackedByteArray(read_bytes(file_position, 14))
		file_position += 14

		var animation_count := read_u32(file_position)
		file_position += 4
		var animations: Dictionary = {}
		for j in range(animation_count):
			var animation_index := read_s32(file_position)
			file_position += 4
			
			# Unknown Bytes
			var animation_unknown_bytes := PackedByteArray(read_bytes(file_position, 4))
			file_position += 4

			var animation_frame_count := read_u32(file_position)
			file_position += 4
			...
			for k in range(animation_frame_count):
				...
				# Unknown Bytes
				var animation_frames_unknown_bytes := PackedByteArray(read_bytes(file_position, 7))
				file_position += 7
				
				animation_frames.append(
					PartAnimationFrame.new(frame_offset, animation_frames_unknown_bytes))
			animations[animation_index] = PartAnimation.new(animation_index, animation_unknown_bytes, animation_frames)
